...
with tab2:
    ...
    # Dynamic height: 80px base + 36px per row, clamped between 460–720
    chart_h = max(460, min(720, 80 + n_cities * 36))

    # Shared layout overrides so both charts are pixel-identical in position
    SHARED_LAYOUT = dict(
        margin=dict(l=20, r=140, t=60, b=50),   # r=140 gives legend breathing room
        legend=dict(
            title_text="Country",
            orientation="v",
            yanchor="top", y=1.0,
        ...
        title=dict(x=0, xanchor="left", pad=dict(l=0, t=0)),
    )
    ...
    with pc1:
        ...
        apply_chart_theme(fig, height=chart_h)
    ...
    with pc2:
        ...
        apply_chart_theme(fig_bubble, height=chart_h)
        fig_bubble.update_layout(**SHARED_LAYOUT)
