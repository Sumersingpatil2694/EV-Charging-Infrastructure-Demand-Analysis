...
with tab2:
    ...
    with filter_col:
        n_cities = st.slider("Top Cities to Show", min_value=10, max_value=50, value=11, step=1)
    with sort_col:
        sort_options = {
    ...
    # Dynamic chart height: 80px base + 38px per row, min 420, max 720
    bar_chart_height = max(420, min(720, 80 + n_cities * 38))
    ...
    with pc1:
        ...
        apply_chart_theme(fig, height=bar_chart_height)
        fig.update_layout(
    ...
    with pc2:
        ...
        apply_chart_theme(fig_bubble, height=bar_chart_height)
    ...
    # Table height: show 11 rows by default (38px per row + 38px header)
    table_height = 38 * 11 + 38
